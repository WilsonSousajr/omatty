...
- [x] **Step 2: Run, expect "has no field or method ListFiles"**
...
- [x] **Step 4: Run**: `go test ./internal/... -race` — PASS.
...
- [x] **Step 2: Run, expect "undefined: review.NewTree"**

- [x] **Step 3: Implement** `tree.go`:
...
- [x] **Step 4: Run**: `go test ./internal/review/ -race` — PASS.
...
- [x] **Step 2: Run, expect "undefined: review.ReadPreview"**

- [x] **Step 3: Implement** `preview.go`:
...
- [x] **Step 2: Run, expect "unknown field Files"**
...
- [x] **Step 4: Run**: `go test ./... -race` — PASS. Watch file sizes: `review.go` and `reviewkeys.go` under 500 lines; `dupl` between `renderEntries` and `renderTree` is the likely hit, fixed by extracting a `window(off, rows, n int) (start, end int)` helper.
...
- [x] **Step 1: The gate** (same command as Task 15).
...
## What Plan B actually did differently

Recorded after execution.

- **`review.Tree.Retouch` was added**, which Task 17 did not plan. The listing and the diff load as two commands and `git ls-files` returns before `git diff`, so the tree is usually built before the touched set exists. Rebuilding it when the diff lands would drop the collapse state under the operator's cursor, so `Retouch` updates the marks in place and `onDiffLoaded` calls it.
- **The tree keeps its own error** (`ReviewPane.TreeErr`). Task 19 wrote a listing failure into the shared `Err`, which blanked the diff view: two of Plan A's tests failed on the first run because opening the diff with no lister configured painted the lister's error over the diff. The two views load independently, so they carry independent errors.
- **`loadFiles` returns nil in the diff view.** Task 19 batched it into every `toggleView`, spending a `git ls-files` on a column that shows no tree.
- `renderPreview` calls the existing **`expandTabs`** rather than inlining its own tab replacement, so the #21 tab-width fix has one home. Verified: every row of the smoke frames is exactly the window width at 100x30 and 160x45, previewing `cmd/omatty/main.go`.
- `reviewTitle` reuses **`sessionTitle(id)`** from `status.go`, which already did the lookup.
- `NewModel` hit `funlen` at 21 lines from two new fields; the three review-column sources share one line rather than gaining a fourth constructor helper.
- The tree also accepts **`r`** to re-list, matching the diff view, and `followSession` carries the view to the next session (a preview degrades to that session's tree).
