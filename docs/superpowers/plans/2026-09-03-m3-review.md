...
and `reviewTitle` returns `"files · " + sess.Title` for the tree and `p.Path` for the preview. Footer: add `Leader + " f files "` before `d diff` in `footer` (recount: 14 more characters, 108 with the leading space; drop the `worktree` label to `wt` to land at 102, then shorten `restart` to `rst`; the target is at most 100 at 100 columns; verify with `TestModel_FooterSpansTheFullWidthOnTheLastLine_issue35`). Add a `treeFooter = "j/k move  enter open  esc back  " + Leader + " f close"`, chosen in `renderFooter` when `m.review.Focused && m.review.View != ViewDiff`.
...
- Known deviations, stated in the spec: anchors carry `Nth` and fall back by hash within the file; `ctrl+c` in the pane returns to the terminal instead of quitting; the footer switches to single spaces so the new keys fit at 100 columns.
