...
func TestView_PlacesTheEmbeddedCursorOnTheWindow_issue106(t *testing.T) {
	...
	if got.X != ui.SidebarWidth+7 || got.Y != 2+3 {
		t.Errorf("cursor at (%d, %d), want (%d, %d)",
			got.X, got.Y, ui.SidebarWidth+7, 2+3)
	}
}

// PaneOrigin is where the caret and the wheel both measure from, so its value
// is pinned here rather than only derived. The sidebar with its hairline, and
// no left border of the pane's own; under the header row and the rule (#128,
// #174).
func TestPaneOrigin_IsTheEmulatorsTopLeftCell_issue106(t *testing.T) {
	x, y := ui.PaneOrigin()

	if x != ui.SidebarWidth || y != 2 {
